...
async function main() {
  ...
  const indexContent = Object.values(coms).map((comName) => `export { default as ${comName} } from './${comName}.vue'`).join('\n')
  await fsp.writeFile(path.join(output, 'index.ts'), indexContent);

  let mapContent = Object.values(coms).map((comName) => `import ${comName} from './${comName}.vue'`).join('\n')
