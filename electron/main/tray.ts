...
const __static = process.cwd();

function createMenuTemplate(win: BrowserWindow): MenuItemConstructorOptions[] {
	return [
		{
			label: "播放",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/play.png")),
			click: () => {
		...
		{
			label: "暂停",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/pause.png")),
			click: () => {
				win.webContents.send("play");
			},
			id: "pause",
		...
		{
			label: "上一首",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/left.png")),
		...
		{
			label: "下一首",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/right.png")),
		...
		{
			label: "循环播放",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/repeat.png")),
			accelerator: "Alt+R",
			click: () => {
		...
		{
			label: "加入喜欢",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/like.png")),
		...
		{
			label: "取消喜欢",
			icon: nativeImage.createFromPath(path.join(__static, "img/icons/unlike.png")),
			accelerator: "CmdOrCtrl+L",
			click: () => {
				win.webContents.send("like");
			},
...
export class TrayEventEmitter extends EventEmitter<{
	updateTooltip: [title: string];
	updatePlayState: [isPlaying: boolean];
	updateLikeState: [isLiked: boolean];
}> {}
...
// 添加左键支持
// 2022.05.17
class YPMTrayLinuxImpl extends TrayEventEmitter {
	tray: Tray;
	win: BrowserWindow;
	template: MenuItemConstructorOptions[];
	contextMenu: Menu;

	constructor(tray: Tray, win: BrowserWindow) {
		super();
		this.tray = tray;
		this.win = win;
	...
	initTemplate(): MenuItemConstructorOptions[] {
	...
	handleEvents() {
		...
		this.on("updateTooltip", (title) => this.tray.setToolTip(title));
		this.on("updatePlayState", (isPlaying) => {
			this.contextMenu.getMenuItemById("play")!.visible = !isPlaying;
			this.contextMenu.getMenuItemById("pause")!.visible = isPlaying;
			this.tray.setContextMenu(this.contextMenu);
		});
		this.on("updateLikeState", (isLiked) => {
...
class YPMTrayWindowsImpl extends TrayEventEmitter {
	tray: Tray;
	win: BrowserWindow;
	template: MenuItemConstructorOptions[];
	contextMenu: Menu;
	...
	handleEvents() {
		...
		this.on("updateTooltip", (title) => this.tray.setToolTip(title));
		this.on("updatePlayState", (isPlaying) => (this.isPlaying = isPlaying));
		this.on("updateLikeState", (isLiked) => (this.isLiked = isLiked));
	}
}

export function createTray(win: BrowserWindow): TrayEventEmitter {
	...
	return isLinux ? new YPMTrayLinuxImpl(tray, win) : new YPMTrayWindowsImpl(tray, win);
}
